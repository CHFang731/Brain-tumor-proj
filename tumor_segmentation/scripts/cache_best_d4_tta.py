...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cache D4 TTA probability maps for the current best ensemble specs.")
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--batch-size", type=int, default=4)
    parser.add_argument("--ref-size", type=int, default=256)
    parser.add_argument("--cache-dir", default="reports/prob_cache_d4_ref256_20260509")
...
def d4_tta_prob(model: torch.nn.Module, x: torch.Tensor) -> torch.Tensor:
    probs = []
    for k in range(4):
        xt = torch.rot90(x, k=k, dims=(2, 3))
        p = torch.sigmoid(model(xt))
        probs.append(torch.rot90(p, k=-k, dims=(2, 3)))

        xt = torch.flip(torch.rot90(x, k=k, dims=(2, 3)), dims=(3,))
        p = torch.sigmoid(model(xt))
        p = torch.rot90(torch.flip(p, dims=(3,)), k=-k, dims=(2, 3))
        probs.append(p)
        ...
        for split_name in ("val", "test"):
            ...
            with torch.no_grad():
                for batch in loader:
                    ...
                    chunks.append(resize_to(d4_tta_prob(model, x).cpu(), ref_hw).half())
