...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep light smoothing of cached ensemble probability maps.")
    ...
    parser.add_argument("--output", default="reports/ensemble_prob_smoothing_20260509.json")
    parser.add_argument("--sigmas", default="0,0.25,0.35,0.50,0.65,0.80,1.0")
    parser.add_argument("--thresholds", default="0.395,0.398,0.399,0.400,0.40025,0.4005,0.401,0.402,0.403,0.405")
    parser.add_argument("--agreement-ks", default="0,3")
...
def smooth_probs(
    ...
    return {
        model_name: {
            split_name: ndimage.gaussian_filter(
                split_probs,
                sigma=(0, 0, sigma, sigma),
            ).astype(np.float32)
            for split_name, split_probs in by_split.items()
        }
        for model_name, by_split in probs.items()
    }
...
def postprocess_metrics_grid(
    ...
    return {
        min_size: {"dice": values["dice"] / n_images, "iou": values["iou"] / n_images}
...
def main() -> None:
    ...
    total = len(sigmas) * len(thresholds) * len(agreement_ks)
    done = 0
    for sigma in sigmas:
        probs_for_votes = smooth_probs(probs, sigma) if args.smooth_votes else probs
        if args.smooth_votes:
            smoothed = {split_name: ensemble_probs(probs_for_votes, split_name) for split_name in ("val", "test")}
        elif sigma > 0:
            smoothed = {
                split_name: ndimage.gaussian_filter(ens[split_name], sigma=(0, 0, sigma, sigma)).astype(np.float32)
                for split_name in ("val", "test")
            }
        else:
            smoothed = ens
        for threshold in thresholds:
            for agreement_k in agreement_ks:
                processed = {}
                for split_name in ("val", "test"):
                    pred = smoothed[split_name] >= threshold
                    votes = vote_mask(probs_for_votes, split_name, threshold, agreement_k)
